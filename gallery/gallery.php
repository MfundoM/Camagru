    ...
    <div class="cam">
        <div class="container">
            ...
            try {
                ...
                if (empty($images)) {
                    ?> <div class="noImg"> <?php
                        echo "No Photos To Display Yet!!!";
                        ?> </div> <?php
                }
                ?>
                <?php foreach($images as $row) { ?>
                    <div class='frame'>
                        <button class="closebnt" onclick="removeImage(this);" value="<?php echo $row['id']; ?>"><img class='close' src='img/close.png' alt='delete' /></button>
                        <div class='pic' id='frm'>
                            <img class="one" src="images/<?php echo $row['img']; ?>" />
                        </div>
                        <div class="txt">
                            <button class="lykbtn" onclick="likeImage(this);" value="<?php echo $row['id']; ?>"><img class="lyk" src="img/like.png"/></button>
                            <?php
                            $nb_likes = query_db("SELECT COUNT(*) FROM likes WHERE galleryid='{$row['id']}'", $db);
                            echo $nb_likes[0][0];
                            ?>
                            <button value="<?php echo $row['id']; ?>"><img class="lyk" src="img/dislike.png"/></button> </div>
